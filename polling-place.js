...
(function (window, undefined) {
    ...
    // PollingPlace.Request has callback members:
    ...
    Request.prototype.start = function (method, url, headers, body) {
        if (this._timer != null) {
            consoleError("PP: start() called on a Request object that is currently running.");
            return;
        }
    ...
    Request.prototype._start = function () {
        ...
        if (this._delayNext) {
            ...
            consoleInfo("PP: polling again in " + delayTime + "ms");
        } else {
    ...
    Request.prototype.retry = function () {
        if (this._tries == 0) {
            consoleError("PP: retry() called on a Request object that has never been started.");
            return;
        }
        if (this._timer != null) {
            consoleError("PP: retry() called on a Request object that is currently running.");
            return;
        }
        this._retry();
    };
    Request.prototype._retry = function () {
        ...
        delayTime += Math.floor(Math.random() * this.maxDelay);
        consoleInfo("PP: trying again in " + delayTime + "ms");
    ...
    Request.prototype._cleanupConnect = function (abort) {
        ...
        if (this._transport == transportTypes.Xhr) {
            consoleInfo("PP: XHR cleanup");
            this._cleanupXhr(this._xhr, abort);
            this._xhr = null;
        } else { // Jsonp
            consoleInfo("PP: json-p " + this._jsonp.id + " cleanup");
    ...
    Request.prototype._startJsonp = function (method, url, headers, body) {
        ...
        consoleInfo("PP: json-p start " + jsonp.id + " " + src);
    ...
    Request.prototype._jsonpCallback = function (result) {
        consoleInfo("PP: json-p " + this._jsonp.id + " finished");
